...
const   apiKey = (  import .meta .env .GEMINI_API_KEY ) _  _ _ _
const   apiBaseUrl = (  import .meta .env .API_BASE_URL ) ? _ _ _  。_ _修剪（）。替换( /\/$/ , '' )  
...
导出 const  startChatAndSendMessageStream = async (历史记录: ChatMessage [ ] , newMessage : string ) => {
  ...
  // 使用sendMessageStream来传输响应
  const 结果=等待聊天。发送消息流（新消息）

  const 编码流=新的ReadableStream  ( {
    异步启动（控制器）{
      const 编码器= new TextEncoder (   )
      for    await    (   const    chunk    of result.stream ) { _   _  
        const    text = 等待块。文本（）
        const 编码=编码器。编码（）
        控制器。入队（编码）
      }
...
Before在每次回复之前，您都需要检查<rule S>并做出符合<rule S>中角色性格和特征的回复。回复还必须遵守 <rule S> 在检查，做出与<规则S>中的人物性格和特征相符的响应。回复还必须遵守 <rule S> 中的字符集。
